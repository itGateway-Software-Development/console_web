...
let intervalId: number | undefined = undefined;
  ...
  const sendResetCode = async () => {
    if (email.value) {
      ...
      successMsg.value = '';
      errorMsg.value = '';
      const res = await userProfileService.sendPwResetCode(email.value);
  ...
  const checkResetCode = async() => {
    if(resetCode) {
      ...
      const res = await userProfileService.checkPwResetCode(payload);
        ...
        askConfirmation().then(async(result) => {
            if(result.isConfirmed) {
                try{
                    ...
                    const res = await userProfileService.resetPassword(payload);
